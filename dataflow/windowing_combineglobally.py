...
class SomeCalculatios(beam.CombineFn):
  def create_accumulator(self):
    return 0.0, 0

  def add_input(self, sum_count, input):
    (sum, count) = sum_count
    return sum + random.randint(0,22), count + 1
...
def run():
    ...
    data_from_source = (p
                    ...
                    | "Window Operation" >> beam.WindowInto(window.FixedWindows(10))                  
                    | "Flatten JSON" >> beam.Map(flatten_data)
                    | "Json Parser" >> beam.Map(parse_json)
                    | "Calculate Metrics" >> beam.CombineGlobally(SomeCalculatios()).without_defaults()
